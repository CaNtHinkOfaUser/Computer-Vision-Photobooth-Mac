...
class VirtualPhotobooth:
    ...
    def __init__(self):
        ...
        if not self.bg_images:
            print(
                f"  -> No images found in {self.bg_folder}. Trying to load default..."
            )
            fallback = cv2.imread("169950_1.jpg")
            if fallback is not None:
                self.bg_images.append(fallback)
            else:
                print("  -> WARNING: No backgrounds available at all!")
